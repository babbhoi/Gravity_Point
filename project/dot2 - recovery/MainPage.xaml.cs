...
namespace dot2
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        //increase speed of game with time
        DispatcherTimer speedincsr = new DispatcherTimer();
        //int count = 0;
       ...
       bool rcircleclsn;
...

        {
            ...
            test.Width = 70;
             */
            speedincsr.Interval = TimeSpan.FromMilliseconds(5000);
            speedincsr.Tick += speedincsr_Tick;
            speedincsr.Start();
        ...
        void speedincsr_Tick(object sender, object e)
        {
            move=move+0.6;
            dmove = dmove + .1;
        }
        ...
        private void createRCircle()
        {
            ...
            if (rnd1.Next(2, 50) > 30) dcrs.dir = 1;
            else dcrs.dir = 0;
            dir.Add(dcrs);
                        ...
                        if (r <= R * R)
                            {
                                ...
                                if (circles.ElementAt(p).type == 2) rccn();
                                        //tnfr.Stop();
        ...
        private void circulartransition()
        {
            ...
            {
                dotY = dotY +10;
                if (dotY > centerY + Radius) dotY = centerY + Radius;
                ...
                if (centerY + Radius == dotY) dotY = dotY + 1;
            }
            else
            {
                avgX.Clear();
        ...
        private void createCircle()
        {
            ...
            if(rnd.Next(0,10)>6)
                {
                    int c1 = rnd.Next(size /5 , size *7/24);
                   ...
                   /* gencircle(can.ActualWidth / 6, height + can.ActualWidth / 3, can.ActualWidth / 6, 1);
                    gencircle(can.ActualWidth / 3 + ActualWidth / 6, height + can.ActualWidth / 3, can.ActualWidth / 6, 1);
                    gencircle( 2 * can.ActualWidth / 3 + ActualWidth / 6, height + can.ActualWidth / 3, can.ActualWidth / 6, 1);
                    */
                }
            else
            {
                Random rnd2 = new Random();
                int c1 = rnd.Next(size / 7, size / 5);
                int c2 = rnd.Next(size / 7, size / 5);
                
                int random=rnd2.Next(0, 100) ;
                if (random < 20)
                {
                    if (random > 10)
                    {
        ...
        private void gencircle(double X,double Y, double R,int i)
        {
            ...
            switch (i)
            {
                case 1: el.Stroke = new SolidColorBrush(Colors.Orange);
                        //el.Fill = new SolidColorBrush(Colors.Orange);
                    break;
                case 0: el.Stroke = new SolidColorBrush(Colors.Yellow);
                    break;
                case 2: el.Stroke = new SolidColorBrush(Colors.Red);
                    break;
